...
public class Robot extends Component {
	...
	private synchronized int moveToNextPathPosition() {
	...
	private Position findFreeNeighbouringPosition() {
		...
		int res = pathFinder.getResolution();
		
		ArrayList<Position> possiblePositions = new ArrayList<Position>();
		possiblePositions.add(new Position(cur.getxCoordinate() + res, cur.getyCoordinate()));
		possiblePositions.add(new Position(cur.getxCoordinate(), cur.getyCoordinate() + res));
		possiblePositions.add(new Position(cur.getxCoordinate() - res, cur.getyCoordinate()));
		possiblePositions.add(new Position(cur.getxCoordinate(), cur.getyCoordinate() - res));
		...
		for (Position pos: possiblePositions) {
			...
			if (!getFactory().hasObstacleAt(shape) && 
					!getFactory().hasMobileComponentAt(shape, currTargetComponent)) {
				return pos;
			}
		}
	...
	public boolean isLivelyLocked() {
		...
		final Component otherRobot = getFactory().getMobileComponentAt(nextPosition,
				this);
